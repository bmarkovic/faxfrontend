# $Id: acsite.m4,v 1.1 2004/01/26 14:40:48 glenn Exp $
...
QT_VER=`grep 'define.*QT_VERSION_STR[[^A-Za-z0-1]]' $QTDIR/include/qglobal.h | perl -p -e 's/\D//g'`
case "${QT_VER}" in
    2*)
        QT_MAJOR="2"
    ;;
...
if test ! -f $MOC ; then
        AC_MSG_ERROR([*** don't have QT program 'moc'])
fi

# uic is the Qt user interface compiler
AC_PATH_PROG(UIC, uic, $QTDIR/bin/uic)
if test ! -f $UIC ; then
        AC_MSG_ERROR([*** don't have QT program 'uic'])
fi
...
AC_PATH_PROG(QEMBED, qembed, $QTDIR/tools/qembed/qembed)
